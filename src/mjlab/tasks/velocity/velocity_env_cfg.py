...
class ObservationIMUCfg:
  ...
  class PolicyCfg(ObsGroup):
    ...
    #   noise=Unoise(n_min=-0.05, n_max=0.05),
    # )
    accelerometer: ObsTerm = term(
      ObsTerm,
      func=mdp.imu_accelerometer,
      noise=Unoise(n_min=-0.2, n_max=0.2),
    )
    gyroscope: ObsTerm = term(
      ObsTerm,
      func=mdp.imu_gyroscope,
      noise=Unoise(n_min=-0.2, n_max=0.2),
    )
    joint_pos: ObsTerm = term(
      ObsTerm,
...
@dataclass
class LocomotionIMUVelocityEnvCfg(ManagerBasedRlEnvCfg):
  scene: SceneCfg = field(default_factory=lambda: SCENE_CFG)
  observations: ObservationIMUCfg = field(default_factory=ObservationIMUCfg)
  actions: ActionCfg = field(default_factory=ActionCfg)
  rewards: RewardCfg = field(default_factory=RewardCfg)
  events: EventCfg = field(default_factory=EventCfg)
  terminations: TerminationCfg = field(default_factory=TerminationCfg)
